...
export const getUserConnections = async (req, res) => {
    try {
        ...
        // Lọc ra các user 'null' (đã bị xóa) khỏi các mảng
        const connections = user.connections.filter(u => u !== null);
        const followers = user.followers.filter(u => u !== null);
        const following = user.following.filter(u => u !== null);

        // Lọc ra các 'pendingConnections' có user 'null'
        const pendingConnectionsRaw = (await Connection.find({ to_user_id: userId, status: 'pending' }).populate('from_user_id'));
        const pendingConnections = pendingConnectionsRaw
            .filter(c => c.from_user_id !== null) // Lọc kết nối có 'from_user_id' không bị null
...
export const acceptConnectionRequest = async (req, res) => {
    try {
        ...
        const toUser = await User.findById(id);
        // Kiểm tra nếu toUser (người gửi) đã bị xóa
        if (toUser) { 
...
export const getUserProfiles = async (req, res) => {
    try {
        ...
        // --- BẮT ĐẦU SỬA LỖI ---
        // Lấy các bài đăng và populate 'user'
        const populatedPosts = await Post.find({ user: profileId }).populate('user');
        
        // Lọc ra các bài đăng có 'user' không bị null (do user đã bị xóa)
        const posts = populatedPosts.filter(post => post.user !== null);
